...
export default function Home() {
  ...
  useEffect(() => {
    ...
    getFavorites().then((result) => {
      if (result.data) setFavorites(result.data.map((p: Product) => p._id));
    });
  }, []);
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-10">
      <h1 className="mb-8 text-3xl font-bold text-slate-900">{t("home.title")}</h1>

      {loading ? (
        <p className="text-slate-500">{t("common.loadingProducts")}</p>
      ) : products.length === 0 ? (
